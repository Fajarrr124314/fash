          ...
          <div class="flex justify-center items-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-200 overflow-auto max-h-[420px]">
              @if($activePreviewPop)
                  <div class="pop-card-preview bg-white shadow-lg border border-slate-300 relative transition-all duration-300 flex flex-col justify-between overflow-hidden"
                       :class="frameSize === 'A5' ? 'pop-card-a5' : (frameSize === 'A4' ? 'pop-card-a4' : 'pop-card-a3')"
                       :style="
                           ...
                           <div class="flex-grow flex flex-col justify-between py-3 px-5 leading-none">
                               ...
                               <div class="price-area-a5">
                                   ...
                                   @if($activePreviewPop['layout_type'] === 'discount_percent')
                                       @php
                                           $p1 = app()->call([$this, 'formatPriceStatic'], ['val' => $activePreviewPop['additional_data']['item1_price'] ?? '']);
                                           $p2 = app()->call([$this, 'formatPriceStatic'], ['val' => $activePreviewPop['additional_data']['item2_price'] ?? '']);
                                       @endphp
                               ...
                               <!-- Empty space instead of footer -->
                               <div class="h-2"></div>
                           </div>
                       @else
                           <!-- ==================== A4 / A3 LAYOUTS (EXISTING) ==================== -->
                           <!-- Header Banner -->
...
<div id="pop-print-area" class="print-only hidden">
    @foreach($previewQueue as $pq)
        <div class="pop-card-preview bg-white relative flex flex-col justify-between overflow-hidden print-card-item-modal {{ $pq['frame_size'] === 'A5' ? 'pop-card-a5' : ($pq['frame_size'] === 'A4' ? 'pop-card-a4' : 'pop-card-a3') }}"
             style="width: {{ $pq['frame_size'] === 'A5' ? '148mm' : ($pq['frame_size'] === 'A4' ? '148mm' : '210mm') }}; 
                    ...
                    margin: 0 auto; page-break-after: always; page-break-inside: avoid; border: none; box-shadow: none; box-sizing: border-box; padding: 0px; font-family: 'Arial Narrow', 'Archivo Narrow', Arial, sans-serif;">
            ...
            @if($pq['frame_size'] === 'A5')
                ...
                <div class="flex-grow flex flex-col justify-between py-3 px-5 leading-none">
                    ...
                    <div class="price-area-a5">
                        ...
                        <!-- 3. Discount Percent Layout (Price Range for A5) -->
                        @if($pq['layout_type'] === 'discount_percent')
                            @php
                                $p1 = app()->call([$this, 'formatPriceStatic'], ['val' => $pq['additional_data']['item1_price'] ?? '']);
                                $p2 = app()->call([$this, 'formatPriceStatic'], ['val' => $pq['additional_data']['item2_price'] ?? '']);
                            @endphp
                            <div class="flex flex-col items-center justify-center gap-1.5 w-full">
                                <div class="price-wrapper-a5">
                                    <span class="price-rp-a5">Rp</span>
