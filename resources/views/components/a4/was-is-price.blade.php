...
new class extends Component
{
    ...
    public $formTitle = 'Tambah POP A4 Was/Is Price';
    public $popId = null;
    ...
    {
        $query = Pop::where('frame_size', 'A4')->where('layout_type', 'was_is_price');
        if ($this->search) {
    ...
    {
        $this->resetForm();
        $this->formTitle = 'Tambah POP A4 Was/Is Price';
        $this->showForm = true;
    }
    ...
    public function save()
    {
        ...
        if ($this->popId) {
            ...
            $msg = 'POP A4 Was/Is Price berhasil diperbarui!';
        } else {
            Pop::create($data);
            $msg = 'POP A4 Was/Is Price berhasil ditambahkan!';
        }
...
<div class="space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-md overflow-hidden flex flex-col w-full">
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h3 class="text-base font-extrabold text-slate-800">POP A4 - Harga Coret</h3>
                <p class="text-xs text-slate-400 font-semibold uppercase mt-0.5">Daftar SKU / Promo (Was/Is A4)</p>
            </div>
                        ...
                        @foreach($pops as $pop)
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="py-3 px-4 text-right font-medium text-slate-500 line-through">
                                    @php
                                        $oldP = app()->call([$this, 'formatPriceStatic'], ['val' => $pop['secondary_price']]);
                                    @endphp
